...
class CuckooFilter:
    ...
    def _get_index(self, item):
        ...
        return self.hashfunc(item,0,False) % self.size
    
    def _get_alt_index(self, fingerprint, index):
        """Get alternate index using fingerprint"""
        return (index ^ self.hashfunc(str(fingerprint).encode(),0,False)) % self.size
...
def test():
	# Create a filter with capacity for 1000 items
	...
	print("apple" in cf)  # True
	print("grape" in cf)  # False (probably)

	# Remove items
	cf.remove("banana")
	print("banana" in cf)  # False
